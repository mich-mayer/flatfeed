...
NONE_RUN_DIR = EVAL_ROOT / "runs" / "luna-effort-screen-none"
LOW_RUN_DIR = EVAL_ROOT / "runs" / "luna-effort-screen-low"
OUTPUT_DIR = EVAL_ROOT / "runs" / "luna-effort-screen-comparison"


def _parse(row: Mapping[str, object]) -> tuple[bool, bool, str | None]:
    ...
    try:
        ...
        return False, False, None
    if set(value) != {"has_error", "error_field"}:
        return False, False, None
    ...
    if error_field is not None and not isinstance(error_field, str):
        return False, False, None
    if has_error != (error_field is not None):
        return False, False, None
    return True, has_error, error_field
...
def _case_correct(
    ...
    covered, has_error, error_field = _parse(prediction)
    if not covered:
        return False, False, None
    if truth["case_type"] == "clean":
        return not has_error, has_error, error_field
    expected = TRUTH_FIELD_TO_ERROR_FIELD[str(truth["corrupted_field"])]
    return has_error and error_field == expected, has_error, error_field
...
def _profile(
    ...
    if set(truth_by_id) != set(prediction_by_id):
        raise ValueError("effort-screen truth and prediction IDs differ")
...
def compare_efforts(
    *,
    ...
    criteria = {
        ...
        "low_adds_at_least_two_correct_wbs": (
            low["correct_wbs"] >= none["correct_wbs"] + 2
        ),
        "low_does_not_increase_false_alerts": (
            low["false_alerts"] <= none["false_alerts"]
        ),
        "low_does_not_reduce_non_wbs_correct": (
            low["correct_non_wbs"] >= none["correct_non_wbs"]
        ),
...
def _markdown(comparison: Mapping[str, object]) -> str:
    ...
    lines = [
        "# Luna Reasoning-Effort Screen",
        "",
        "Development diagnostic only; not calibration or validation evidence.",
        "",
        "| Measure | none | low |",
        "|---|---:|---:|",
        f"| Correct WBS | {none['correct_wbs']}/10 | {low['correct_wbs']}/10 |",
        f"| Clean false alerts | {none['false_alerts']}/8 | {low['false_alerts']}/8 |",
        f"| Correct non-WBS | {none['correct_non_wbs']}/6 | {low['correct_non_wbs']}/6 |",
        f"| Total correct | {none['total_correct']}/24 | {low['total_correct']}/24 |",
        ...
        *(f"- {name}: {'pass' if passed else 'fail'}" for name, passed in criteria.items()),
        ...
        "",
        "The locked holdout was not used.",
        "",
    ]
    return "\n".join(lines)
...
def main() -> None:
    truth_rows = load_jsonl(DEFAULT_LUNA_EFFORT_SCREEN_DIR / "truth.jsonl")
    comparison = compare_efforts(
