...
DEFAULT_LUNA_EFFORT_SCREEN_DIR = (
    Path(__file__).with_name("datasets") / "luna_effort_screen"
)
SCREEN_COUNTS = {"clean": 8, "corrupted": 16}
...
SCREEN_SPLIT = "luna_effort_screen"
...
def _family_order(seed: int) -> list[str]:
    ...
    random.Random(_derived_seed(seed, "screen-clean-families")).shuffle(clean)
    random.Random(_derived_seed(seed, "screen-wbs-families")).shuffle(wbs)
    random.Random(_derived_seed(seed, "screen-non-wbs-families")).shuffle(non_wbs)
    corrupted: list[str] = []
    wbs_index = 0
    non_wbs_index = 0
...
def _find_family(raw_text: str) -> str:
    ...
    if len(matches) != 1:
        raise ValueError("screen row lacks exactly one registered WBS phrase")
    return matches[0]


def build_luna_effort_screen_rows(
    *,
    ...
    source_cases = generate_clean_ai_qa_cases(
        count=len(families),
        seed=_derived_seed(seed, "screen-clean-source-cases"),
    )
    occurrences: Counter[str] = Counter()
    specialized = []
    for case, family in zip(source_cases, families):
        phrases = LUNA_V5_WBS_PHRASES[family]
        phrase = phrases[occurrences[family] % len(phrases)]
...
def _verify_composition(
    ...
    for input_row, truth_row in zip(input_rows, truth_rows):
        if input_row["case_id"] != truth_row["case_id"]:
            raise ValueError("screen input/truth order mismatch")
        family = _find_family(str(input_row["raw_text"]))
        if truth_row["case_type"] == "clean":
            clean_families[family] += 1
            continue
        expected_field = TRUTH_FIELD_TO_ERROR_FIELD[str(truth_row["corrupted_field"])]
        if expected_field == "wbs":
            wbs_error_families[family] += 1
        else:
            non_wbs_fields[expected_field] += 1
    if clean_families != Counter(CLEAN_FAMILIES):
        raise ValueError("screen clean WBS composition mismatch")
    if wbs_error_families != Counter(WBS_ERROR_FAMILIES):
        raise ValueError("screen WBS-error composition mismatch")
    expected_non_wbs = Counter(
        field for field, count in SCREEN_ERROR_DISTRIBUTION.items() if field != "wbs" for _ in range(count)
    )
    if non_wbs_fields != expected_non_wbs:
        raise ValueError("screen non-WBS composition mismatch")
    return {
...
def _verify_isolation(input_rows: Sequence[Mapping[str, object]]) -> dict[str, int]:
    ...
    if any(overlaps.values()):
        raise ValueError(f"effort screen overlaps prior data: {overlaps}")
    return overlaps


def write_luna_effort_screen(
    output_dir: Path = DEFAULT_LUNA_EFFORT_SCREEN_DIR,
) -> dict[str, object]:
    input_rows, truth_rows = build_luna_effort_screen_rows()
    ...
    manifest = {
        ...
        "experiment": "synthetic offline Luna reasoning-effort screen",
        "seed": SCREEN_SEED,
        "split": SCREEN_SPLIT,
        ...
        "selection_rule": {
            ...
            "minimum_additional_correct_wbs_for_low": 2,
            "maximum_false_alert_increase": 0,
            "minimum_non_wbs_correct_delta": 0,
            "minimum_total_correct_delta": 0,
        },
        "boundaries": {
    ...
    verify_luna_effort_screen(output_dir)
    return manifest


def verify_luna_effort_screen(
    output_dir: Path = DEFAULT_LUNA_EFFORT_SCREEN_DIR,
) -> dict[str, object]:
    manifest_path = output_dir / "dataset_manifest.json"
    ...
    if _sha256_file(input_path) != manifest["model_inputs"]["sha256"]:
        raise ValueError("effort-screen input hash mismatch")
    if _sha256_file(truth_path) != manifest["truth"]["sha256"]:
        raise ValueError("effort-screen truth hash mismatch")
    ...
    if _verify_composition(input_rows, truth_rows) != manifest["composition"]:
        raise ValueError("effort-screen composition metadata mismatch")
    if _verify_isolation(input_rows) != manifest["isolation"]["overlaps"]:
        raise ValueError("effort-screen isolation metadata mismatch")
    return manifest


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate Luna effort-screen data.")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_LUNA_EFFORT_SCREEN_DIR)
    parser.add_argument("--verify-only", action="store_true")
    args = parser.parse_args()
    manifest = (
        verify_luna_effort_screen(args.output_dir)
        if args.verify_only
        else write_luna_effort_screen(args.output_dir)
    )
    print(
        json.dumps(
            {
