...
SCHEMA_VERSION = "1.0"
SCREEN_SEED = 20260822
SCREEN_SPLIT = "terra_v2_prompt_screen"
...
NON_WBS_ERROR_FAMILIES = tuple(
    family
    for family in LUNA_V5_WBS_PHRASES
    for _ in range(4)
)
...
def _family_order(seed: int) -> list[str]:
    ...
    random.Random(_derived_seed(seed, "terra-v2-clean-families")).shuffle(clean)
    random.Random(_derived_seed(seed, "terra-v2-wbs-families")).shuffle(wbs)
    random.Random(_derived_seed(seed, "terra-v2-non-wbs-families")).shuffle(
        non_wbs
    )
    corrupted: list[str] = []
    wbs_index = 0
...
def build_terra_v2_screen_rows(
    *,
    ...
    source_cases = generate_clean_ai_qa_cases(
        count=len(families),
        seed=_derived_seed(seed, "terra-v2-source-cases"),
    )
    occurrences: Counter[str] = Counter()
    specialized = []
    for case, family in zip(source_cases, families):
        phrases = LUNA_V5_WBS_PHRASES[family]
        offset = _derived_seed(seed, f"terra-v2-phrase:{family}") % len(phrases)
...
def _find_family(raw_text: str) -> str:
    ...
    if len(matches) != 1:
        raise ValueError("Terra-v2 screen row lacks one registered WBS phrase")
    return matches[0]
...
def _verify_composition(
    ...
    for input_row, truth_row in zip(input_rows, truth_rows):
        if input_row["case_id"] != truth_row["case_id"]:
            raise ValueError("Terra-v2 input/truth order mismatch")
    ...
    if clean_families != Counter(CLEAN_FAMILIES):
        raise ValueError("Terra-v2 clean WBS composition mismatch")
    if wbs_families != Counter(WBS_ERROR_FAMILIES):
        raise ValueError("Terra-v2 WBS-error composition mismatch")
    if non_wbs_families != Counter(NON_WBS_ERROR_FAMILIES):
        raise ValueError("Terra-v2 non-WBS composition mismatch")
    ...
    if non_wbs_fields != expected_non_wbs:
        raise ValueError("Terra-v2 non-WBS field composition mismatch")
    return {
...
def _prior_input_paths() -> list[Path]:
    return [
        *_prior_paths(),
        DEFAULT_TERRA_EFFORT_SCREEN_DIR / "model_inputs.jsonl",
        DEFAULT_TERRA_PROMPT_REASONING_SCREEN_DIR / "model_inputs.jsonl",
        DEFAULT_TERRA_CALIBRATION_DATASET_DIR / "calibration_model_inputs.jsonl",
        DEFAULT_TERRA_CALIBRATION_DATASET_DIR / "validation_model_inputs.jsonl",
    ]
...
def _verify_isolation(
    ...
    overlaps = {
        ...
        for path in _prior_input_paths()
    }
    if any(overlaps.values()):
        raise ValueError(f"Terra-v2 screen overlaps prior data: {overlaps}")
    return overlaps
...
def write_terra_v2_screen(
    output_dir: Path = DEFAULT_TERRA_V2_SCREEN_DIR,
) -> dict[str, object]:
    if any(
        (output_dir / name).exists()
        for name in (MODEL_INPUTS_FILE, TRUTH_FILE, "dataset_manifest.json")
    ):
        raise FileExistsError("Terra-v2 screen artifacts already exist")
    inputs, truth = build_terra_v2_screen_rows()
    ...
    manifest = {
        ...
        "hypothesis": (
            "A task-general equality ledger and final rooms/WBS contradiction "
            "veto reduce direct comparison misses without increasing false alerts."
        ),
        "seed": SCREEN_SEED,
        "split": SCREEN_SPLIT,
        ...
        "advancement_rule": {
            "absolute": {
                ...
                "minimum_correct_rooms": 19,
                "minimum_correct_wbs": 19,
                "maximum_clean_false_alerts": 1,
                "minimum_correct_other_fields": 7,
                "minimum_total_correct": 61,
            },
            "comparative": {
                "minimum_additional_correct_rooms_vs_v1": 1,
    ...
    verify_terra_v2_screen(output_dir)
    return manifest


def verify_terra_v2_screen(
    output_dir: Path = DEFAULT_TERRA_V2_SCREEN_DIR,
) -> dict[str, Any]:
    manifest = json.loads(
    ...
    if _sha256_file(input_path) != manifest["model_inputs"]["sha256"]:
        raise ValueError("Terra-v2 input hash mismatch")
    if _sha256_file(truth_path) != manifest["truth"]["sha256"]:
        raise ValueError("Terra-v2 truth hash mismatch")
    ...
    if _verify_composition(inputs, truth) != manifest["composition"]:
        raise ValueError("Terra-v2 composition metadata mismatch")
    if _verify_isolation(inputs) != manifest["isolation"]["overlaps"]:
        raise ValueError("Terra-v2 isolation metadata mismatch")
    return manifest


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate the independent Terra-v2 prompt screen.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_TERRA_V2_SCREEN_DIR,
    )
    parser.add_argument("--verify-only", action="store_true")
    args = parser.parse_args()
    manifest = (
        verify_terra_v2_screen(args.output_dir)
        if args.verify_only
        else write_terra_v2_screen(args.output_dir)
    )
    print(
        json.dumps(
            {
