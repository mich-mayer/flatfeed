...
- **Rationale:** readers need the model, dataset split, evidence contract and run conditions before interpreting the results. Put that context in one scorecard introduction and retain the unusable-result explanation in the existing closed method details, removing the repeated boundary panel.
