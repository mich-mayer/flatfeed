...
The page MUST answer in order: what user problem the product simplifies, what was built, what the candidate owned and decided, how the bounded AI check was evaluated, and what should be tested next. Product combines the solution, four-stage workflow and seven-screen Telegram proof. Decisions combines the three consequential trade-offs. AI Evaluation opens with a concise prose boundary between rule-based matching and the separate offline parser-quality check, then keeps the aggregate results visible; field-level results, configuration and cost assumptions sit in closed-by-default details so they do not interrupt the hiring scan. Dashboard mockups, mock-cost, legal analysis, scripted-demo history and historical iteration tables do not belong in the main public narrative. At ≤56rem the header is not sticky and the nav moves to a second row as a two-column grid; the brand and Repository action remain in the first row without creating an implicit third column.
...
- **Product:** combine the solution and implementation proof. Begin with a compact product-status line, then the four-stage implemented flow—save, normalize each source into one listing format, match with rules, return matches in Telegram—and the seven-screen carousel. The `Save one filter` step gives the first accessible Kaltmiete tooltip. Explain that structured feeds can map fields directly while text-based or incomplete sources require extraction, and identify the current prototype as one text-based path over generated data. Explain the user-visible action before technical architecture. Do not repeat the workflow in comparison or capability cards.
- **Decisions:** use a full-width heading, then a full-width `My role` note before the three decision cards. The note names direct ownership and coding collaborators without competing with the heading. Show three choices in reader language: evaluate AI as a conditional quality check for one plausible text-based ingestion path, use synthetic data to test product mechanics while source access and reuse terms remain unvalidated, and do not match listings when required details are missing. The first card may state the portfolio-learning purpose, but MUST frame parser and AI QA as contingent on source format rather than as fixed production architecture. Scripted-demo history does not belong in the linear case.
- **AI Evaluation:** begin after the `04 AI Evaluation` kicker with one heading and one prose paragraph that scope the check to the prototype's text-based ingestion path: AI returns source evidence, code compares it with parsed values, differences go to a configured admin, and matching stays rule-based. State that complete, reliable structured fields could bypass this path. Present this as an architecture boundary, never as an apology. Then move directly to one compact three-row worked example under a field-neutral heading: the first row identifies the synthetic German `Listing text` and contains all four matching values, followed by aligned `Parser output` and `AI evidence` rows that show values only. Treat every source label as a bordered table cell and keep all three rows equal in height at each viewport. Highlight the differing value, explain the resulting risk to a user and state the admin-review step at the same visible depth: AI points out the conflict, while only a configured admin checks the original listing and labels the finding as parser error, parser correct or unsure before any data changes. Users never see or act on AI flags. The example may use a WBS range without implying that WBS is the only field that can be wrong. Then move to the question of how well the check flags parser errors. Before the aggregate metrics, name the tested model, reasoning effort and response format; describe the frozen set as 300 agreeing pairs and 300 with one deliberately altered structured value; and retain the synthetic qualifier and no-retry/no-tuning setup at visible depth. Keep the 599/600 usable-results limitation in the aggregate metrics without a separate case explanation. Field-level results, configuration and cost scenario remain in closed details. These values are controlled evaluation evidence, never user or production outcomes. No score from an earlier model iteration appears.
- **Next:** show four next product metrics: source access and format, fast delivery, user value and a conditional review loop. Source validation asks whether FlatFeed can secure a permitted data channel from each relevant provider and confirm what format the data comes in. Fast delivery asks how quickly newly published listings reach FlatFeed. The review-loop card is explicitly conditional on a source requiring text extraction. The AI boundary remains explained in AI Evaluation; do not repeat it in Next. The prototype limits carry the remaining scope caveats together: generated listing text, disabled delivery and runtime AI checks, and offline evaluation rather than real-provider data. Do not imply a committed user study, live-source pilot or production rollout.
- **Conclusion:** explicitly synthesize the hiring signal in a dark concluding summary: a working apartment-search workflow, rules making user-facing matches, one evaluated way to process text-based listings with bounded AI QA, and a production design that follows actual provider access terms, formats and data quality. Do not add a kicker above its headline. The final summary has no button; product behavior is demonstrated through screenshots.
...
### 2026-08-21 clarify public case-study language

- **Problem:** reader review found several accurate but internal phrases—such as `ingestion`, `fail closed` and `turn ... into`—hard to understand at a glance. The delivery and model-selection copy also did not make their intended sequence explicit enough.
- **Rationale:** replace internal vocabulary with plain descriptions of what happens: listings reach FlatFeed, incomplete listings do not match, and review can help improve the check. Describe automatic Telegram delivery after a saved filter as product behavior. State the model-evaluation chronology unambiguously: metric targets before model testing, comparison on development data, then one locked evaluation.
- **Affected surfaces:** `docs/case-study.html`, `CASE_STUDY.md`, `scripts/check_eval_numbers.py`, and this file (§§22, 27 and 34).
- **Compatibility impact:** public copy using the replaced technical phrases no longer conforms. The synthetic-data boundary, deterministic matching, conditional source-format architecture, admin-only AI QA, and frozen evaluation evidence remain unchanged.
- **Migration consideration:** update the HTML and Markdown case studies together; perform the 10s/45s scan, desktop/mobile render checks, and Pages verification after publication.

### 2026-08-20 make parser and AI QA conditional on source format

- **Problem:** the case study presented deterministic text parsing and AI QA as a universal FlatFeed architecture even though the required ingestion path depends on the permitted channel, format and field guarantees each housing provider can offer.
- **Rationale:** preserve the stable product core—one listing format, rule-based matching and Telegram delivery—while showing two possible acquisition paths. Complete structured feeds can map fields directly; text-based or incomplete sources may require extraction and bounded AI review. The self-directed portfolio rationale belongs in Decisions so it demonstrates how the author scopes and constrains AI without making AI a product dependency.
- **Affected surfaces:** `docs/case-study.html`, `CASE_STUDY.md`, and this file (§22 and §34).
- **Compatibility impact:** copy that treats the parser or AI QA as required for every provider source no longer conforms. Product status, prototype limits, Decision A, the AI Evaluation introduction, Next and the conclusion must preserve the conditional boundary at their respective reading depths.
- **Migration consideration:** fixed locally for sandbox review. Product behavior, evidence numbers and evaluation artifacts are unchanged; do not publish until the author explicitly requests production deployment.

### 2026-08-20 simplify the Decisions ownership note

- **Problem:** the mono scope label below `My role` repeated portfolio context already stated in the AI decision and interrupted the ownership note with a negative rollout claim.
- **Rationale:** the reader only needs the ownership statement here. The prototype boundary is established elsewhere through synthetic-data and source-access qualifiers, while the decision card retains the self-directed portfolio rationale for the evaluated AI path.
- **Affected surfaces:** `docs/case-study.html`, `CASE_STUDY.md`, and this file (§§22 and 34).
- **Compatibility impact:** a standalone `Self-directed portfolio project · Not a live rollout` label below the ownership note no longer conforms.
- **Migration consideration:** fixed locally for sandbox review; no product behavior, evidence claim or evaluation result changes.

### 2026-08-20 consolidate Next validation cards

- **Problem:** the Next section measured source coverage rather than the prerequisite of lawful source access, treated formal matching as user value, and separated the review-loop learning question into a duplicate data-quality note.
- **Rationale:** the four cards should name the future pilot questions directly: lawful access, delivery speed, user value and whether review leads to parser fixes. The existing AI Evaluation already explains the admin-only, non-mutating boundary, so repeating it in Next adds no reader value.
- **Affected surfaces:** `docs/case-study.html`, `CASE_STUDY.md`, and this file (§§22 and 34).
- **Compatibility impact:** the old Source coverage, Time to alert, Alert follow-through and Irrelevant or unavailable listings cards, plus the separate Data-quality gate for live listings panel, no longer conform.
- **Migration consideration:** fixed now. Keep the four-card layout and preserve the AI boundary in AI Evaluation when revising the Next section.

### 2026-08-20 align full-width section-heading measures

- **Problem:** Product and Next use a two-column heading grid even when no explanatory paragraph occupies the second column. Their headings were constrained to the left column, while Decisions already used the full grid width. The long Next validation question became needlessly dense at desktop width.
- **Rationale:** headings without companion prose should use the same readable measure. Product, Decisions and Next therefore share a `60rem` maximum measure, while AI Evaluation retains its two-column heading-and-explanation layout.
- **Affected surfaces:** `docs/case-study.html`, `docs/styles.css`, and this file (§§6, 14 and 34).
- **Compatibility impact:** only the desktop reading measure of the three section headings changes. Copy, section order, metric cards and responsive single-column behavior remain unchanged.
- **Migration consideration:** fixed now. Verify Product and Next at desktop and mobile widths; do not alter the AI Evaluation heading layout.

### 2026-08-20 consolidate evaluation context before the scorecard

- **Problem:** the aggregate metrics appeared before the tested model and response format were named, while a large Evidence Boundary block repeated the synthetic and offline limitations after the metrics.
- **Rationale:** readers need the model, dataset split, evidence contract and run conditions before interpreting the results. Put that context in one scorecard introduction, then show the field breakdown without repeating the method or the single unusable case.
- **Affected surfaces:** `docs/case-study.html`, `docs/styles.css`, `CASE_STUDY.md`, `scripts/check_eval_numbers.py`, and this file (§§22, 23 and 34).
- **Compatibility impact:** the scorecard question now uses `But` to connect the worked example to the aggregate evaluation; a scorecard that omits the tested configuration or restores a separate Evidence Boundary panel no longer conforms.
- **Migration consideration:** fixed locally across the HTML and Markdown case studies and the public-evidence sync contract. Before publication, run the eval-number sync check, verify desktop and mobile rendering, and keep the change unpublished until the author explicitly requests production deployment.

### 2026-08-20 remove the duplicated AI runtime note

- **Problem:** the stacked AI runtime note repeated the process already explained in the evaluation introduction, while its internal labels added jargon rather than clarity.
- **Rationale:** the introduction now states the boundary in two plain sentences, then moves straight to the worked example. This preserves the reader's understanding of what AI can and cannot decide without restating the same pipeline.
- **Affected surfaces:** `docs/case-study.html`, `CASE_STUDY.md`, and this file (§22).
- **Compatibility impact:** the separate runtime note and its scope label no longer conform; the worked example immediately follows the AI Evaluation introduction.
- **Migration consideration:** verify that the example remains clearly separated from the introduction at desktop and mobile widths.
